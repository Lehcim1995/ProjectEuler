...
namespace adventofcode2019.Days
{
    public class Day15 : IProblem
    {
        ...
        private int NewMove(Point location, int direction)
        {
            ...
            // right hand movement though thingy
            switch(direction)
            {
                case 1: // north
                    if (map[location + east] != 1)
                        return 4;

                    if (map[location + west] != 1)
                        return 3;

                    if (map[location + north] != 1)
                        return 1;
        ...
        private void DiscoverAround(Point location)
        {
            Point north = new Point(0, 1);
            Point east = new Point(1, 0);
            Point south = new Point(0, -1);
